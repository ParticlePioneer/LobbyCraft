...
def get_waiting_parties_all(mode_id: int) -> list:
    """
    Returns ALL waiting parties for a mode with no MMR filter.
    Used by BucketEngine and any engine that handles its own
    candidate filtering internally.
    ...
    sql = '''
        SELECT q.queue_no, q.party_id, q.enqueue_time,
               AVG(p.current_mmr)  AS avg_mmr,
               LISTAGG(p.player_id, ',')
                   WITHIN GROUP (ORDER BY p.player_id) AS pids
        FROM   QUEUE q
        JOIN   PARTY_MEMBER pm ON pm.party_id = q.party_id
        JOIN   PLAYER p        ON p.player_id  = pm.player_id
        WHERE  q.status  = 'WAITING'
